...
const PartnershipDetails: React.FC = () => {
  ...
  const [selectedPartnerDetails, setSelectedPartnerDetails] = useState<PartnerDetails | null>(
    null
  );
  ...
  useEffect(() => {
    axios.get('/api/seller-in-details').then((res) => {
      setPartnerInfo(res.data.partner_information);
    });
  }, []);

  useEffect(() => {
    const fetchSellerPartnerData = async () => {
      try {
        ...
        setCompanyInfo({
          ...
          general_contact: overview.hq_address || '',
          company_email: overview.email || '',
          contact_number: overview.phone || '',
          partner_website: overview.website || '',
        ...
        showAlert({ type: 'error', message: 'Failed to fetch seller partner data' });
      }
    };

    if (id) {
      fetchSellerPartnerData();
    }
  }, [id]);
  ...
  useEffect(() => {
    const fetchPartnerDetails = async (partnerId: string | null) => {
      if (!partnerId) return;
    ...
    fetchPartnerDetails(partnerId ?? null);
  }, [partnerId]);
  ...
  const partnerCountryData = getCountryById(
    parseInt(selectedPartnerDetails?.partner_overview?.hq_country || '0')
  );
                ...
                <div className="flex justify-start items-start flex-col gap-4 w-[344px]">
                  ...
                  <div className="flex self-stretch justify-start items-center flex-row gap-[49px]">
                    ...
                    <div className="flex justify-between items-center flex-row gap-[49px] w-[54px] ml-[30px]">
                      <span className="text-[#30313D] text-sm font-semibold leading-5 flex items-center tex">
              ...
              <div className="flex justify-start items-start flex-col gap-[13px] w-[520px]">
                ...
                <div className="flex justify-start items-start flex-col gap-4 w-[344px]">
                  ...
                  <div className="flex self-stretch justify-start items-center flex-row gap-[49px]">
                    <div className="text-[#727272] text-sm font-medium leading-5">Seller Rank</div>
                    <div className="flex justify-between items-center flex-row gap-[49px] w-[54px] ml-[30px]">
                      <span className="text-[#30313D] text-sm font-semibold leading-5 flex items-center">
                        {companyInfo?.company_rank || 'N/A'}
                      </span>
                      <div>
                        {[...Array(companyInfo?.rating || 0)].map((_, i) => (
              ...
              <div className="flex self-stretch justify-start items-start flex-col gap-[41px]">
                ...
                >
                  ...
                  <p className="self-stretch text-[#000000] text-sm">
                    ...
                    <br /> affiliation or association with external entities.
                  </p>
                </div>
              </div>
