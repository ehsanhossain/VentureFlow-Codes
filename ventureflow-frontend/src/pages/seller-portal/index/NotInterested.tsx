...
interface ApiIndustry extends Industry {
  status?: string;
...
const NotInterested = ({
  ...
  useEffect(() => {
    ...
    const selectedIds = selectedIndustries.map((ind: Industry) => ind.id);
  ...
  useEffect(() => {
    const fetchCurrencies = async () => {
      try {
        ...
        const currencies: { id: number; currency_name: string }[] = Array.isArray(response.data.data)
          ? response.data.data
          : response.data;
  ...
  const [filters, setFilters] = useState<FilterState | Record<string, unknown>>(initialState || defaultInitialState);
  ...
  const fetchSellers = React.useCallback(
    async (page = 1, filtersArg: Record<string, unknown> = {}) => {
      try {
        setLoading(true);

        const params: Record<string, unknown> = { page, search: searchQuery, ...filtersArg };

    

        const response = await api.get('/api/seller/unpinned', { params });
  ...
  const [selectedSellerIds, setSelectedSellerIds] = useState(new Set<number | string>());
  ...
  useEffect(() => {
    api
      .get('/api/countries')
      .then((res) => {
        const mappedCountries = res.data.map((c: { id: number; name: string; svg_icon_url?: string }) => ({
          ...c,
          flagSrc: c.svg_icon_url,
          status: 'registered', // Default status as Dropdown expects it
        }));
  ...
  const getCountryById = (id: number | string) => countries.find((c) => c.id === id);
