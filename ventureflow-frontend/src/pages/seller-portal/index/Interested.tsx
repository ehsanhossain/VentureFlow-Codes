...
const Interested = ({
  ...
  useEffect(() => {
    const fetchIndustries = async () => {
      try {
        ...
        const formatted = response.data.map((industry: { id: number; name: string; status: string; sub_industries: ExtendedIndustry[] }) => ({
          id: industry.id,
          name: industry.name,
  ...
  useEffect(() => {
    ...
    const subs = industries
      ...
      .flatMap((ind) => {
        const industryObj = ind as ExtendedIndustry;
        return industryObj.sub_industries || [];
      });
  ...
  const handleApplySearch = (formValues: FormValues) => {
    ...
    const appliedFilters: FilterState = {
      registered: registeredAfter,
      country: typeof formValues.HQCountry === 'string' ? formValues.HQCountry : '',
      structure: formValues.partnershipStructure || '',
      status: formValues.status || '',
      broderIndustries: formValues.broderIndustries || [],
      priorityIndustries: formValues.priorityIndustries || [],
    };
  ...
  const fetchSellers = useCallback(async (
    ...
    try {
      ...
      const params: Record<string, string | number | undefined> = { page, search: searchQuery, ...filters };
      if (sort) params.sort = sort;

 

      const response = await api.get('/api/seller/pinned', { params });
  ...
  }, [currentPage, fetchSellers, filters]);
  ...
  const sellerData = useMemo(
    () => [
      {
        seller_name: 'Kiran Co Ltd.',
        countryFlag: '',
        hq: 'Italy',
        industry: 'Wholesale',
        share: '~60%',
        expectations: '₺110M',
        dealerroom_id: 'BN-S-675',
  ...
  const [sortConfig, setSortConfig] = useState<{
    key: keyof (typeof sellerData)[0];
    direction: 'asc' | 'desc';
  } | null>(null);

  // sortConfig is maintained for future sorting implementation when API returns real data
  void sortConfig;

  const handleSort = (key: keyof (typeof sellerData)[0]) => {
    setSortConfig((prev) =>
      prev?.key === key
  ...
  const handleBulkDeleteClick = async () => {
    const idsToDeleteArray = Array.from(selectedSellerIds) as (string | number)[];
  ...
  useEffect(() => {
    api
      .get('/api/countries')
      .then((res) => {
        const formatted = res.data.map((c: { id: number; name: string; svg_icon_url?: string }) => ({
          ...c,
  ...
  const getCountryById = (id: string | number) => {
    if (!id) return null;
    return countries.find((c: Country) => c.id === id || String(c.id) === String(id));
  };
                            ...
                            render={({ field }) => (
                              <div className="w-[300px]">
                                <Dropdown
                                  {...field}
                                  countries={countries}
                                  selected={typeof field.value === 'string' ? field.value as unknown as Country : field.value}
                                  onSelect={field.onChange}
                                />
                              </div>
                            )}
            ...
            <TableHeader>
              <TableRow className="rounded-lg">
                {tableHeaders.map((header, idx) => {
                  const key = Object.keys(sellerData[0])[idx] as
                    | keyof (typeof sellerData)[0]
                    | undefined;
              ...
              {loading ? (
                ...
                sellers.map((seller, index) => {
                  ...
                  const countryData = getCountryById(
                    parseInt(String(seller.company_overview?.hq_country))
                  );
                  ...
                  return (
                    <TableRow key={seller.id || index}>
                      ...
                      >
                        {(() => {
                          try {
                            const industries = seller.company_overview?.industry_ops;
                            if (!Array.isArray(industries)) return 'N/A';

                            const truncate = (text: string, limit = 12) =>
                      ...
                      >
                        {seller.financial_details?.maximum_investor_shareholding_percentage ??
                          'N/A'}
                      </TableCell>
                      <TableCell
                        className={clsx(
                      ...
                      >
                        {seller.financial_details?.expected_investment_amount ?? 'N/A'}
                      </TableCell>
                      <TableCell
                        className={clsx(
                      ...
                      >
                        <div className="flex items-center justify-center gap-3">
                          <button
                            type="button"
                            className="flex items-center justify-center w-[35px] h-[35px] bg-[#FFF7F7] border border-[#DF272A] rounded-full p-1"
                          ...
                          >
                            ...
                            >
                              <path
                                d="M2.14867 5.23529V16.8824C2.14867 18.0519 3.09677 19 4.26631 19H12.7369C13.9065 19 14.8545 18.0519 14.8545 16.8824V5.23529M2.14867 5.23529H1.08984M2.14867 5.23529H4.26631M14.8545 5.23529H15.9134M14.8545 5.23529H12.7369M4.26631 5.23529V3.11765C4.26631 1.9481 5.21442 1 6.38396 1H10.6193C11.7888 1 12.7369 1.9481 12.7369 3.11765V5.23529M4.26631 5.23529H12.7369M6.38396 9.47059V14.7647M10.6193 9.47059V14.7647"
                                stroke="#DF272A"
                                strokeWidth="1.6"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                          ...
                          <button
                            type="button"
                            className="flex items-center justify-center w-[35px] h-[35px] bg-white border border-[#064771] rounded-full p-1"
