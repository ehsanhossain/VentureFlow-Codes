...
type FilterValue = string | number | boolean | string[] | number[] | Industry[] | undefined | null;
...
const Drafts = ({
  ...
  useEffect(() => {
    const fetchCurrencies = async () => {
      try {
        ...
        const currencies = Array.isArray(response.data.data)
          ? response.data.data
          : response.data;
  ...
  const [filters, setFilters] = useState<Record<string, FilterValue>>(
    initialState || (defaultInitialState as unknown as Record<string, FilterValue>)
  );
  ...
  const handleApplySearch = (formValues: FormValues) => {
    ...
    const appliedFilters: Record<string, FilterValue> = {
      ...
      maximum_investor_shareholding_percentage:
        formValues.maximumInvestorSharing || "",
    ...
    if (formValues.broderIndustries?.length) {
      appliedFilters.broader_industries = formValues.broderIndustries.map(
        (item) => item.id
      );
    }
    if (formValues.priorityIndustries?.length) {
      appliedFilters.priority_industries = formValues.priorityIndustries.map(
        (item) => item.id
      );
    }
  ...
  const fetchSellers = useCallback(async (
    ...
    try {
      ...
      if (sort) (params as Record<string, string | number>).sort = sort;


      const response = await api.get("/api/seller/drafts", { params });
  ...
  const handleSort = (_key: keyof (typeof sellerData)[0]) => {
      ...
      <div className="flex justify-between items-center px-[20px] ">
        ...
        <div className="flex justify-start items-center gap-4 ">
          <span className="text-sm leading-5 font-poppins text-center">
            ...
            <span className="text-gray-800 font-poppins mx-1">
              {sellers?.length}
            </span>
...
>
                  ...
                  <div className="flex flex-col items-start gap-6 w-full">
                    ...
                    <div className="flex items-center justify-end gap-4 w-full">
                      <div className="flex items-center space-x-3 mr-auto">
                        ...
                        <span className="text-sm font-medium text-[#064771]">
  ...
  <path fillRule="evenodd" clipRule="evenodd" d="M0.782523 2.39433C0.405826 2.0195 0.405826 1.4118 0.782523 1.03698C1.15922 0.662154 1.76997 0.662154 2.14667 1.03698L8.21774 7.07781C8.59409 7.45229 8.59409 8.05943 8.21774 8.43391L2.14667 14.4747C1.76997 14.8496 1.15922 14.8496 0.782523 14.4747C0.405826 14.0999 0.405826 13.4922 0.782523 13.1174L6.17089 7.75586L0.782523 2.39433Z" fill="white"/>
</svg>
                      </button>
                    </div>
                  </div>
            ...
            <TableHeader>
              <TableRow className="rounded-lg">
                {tableHeaders.map((header, idx) => {
                  const _key = Object.keys(sellerData[0])[idx] as
                ...
                sellers.map((seller, index) => {
                  ...
                  return (
                    <TableRow key={seller.id || index}>
                      ...
                      >
                        {seller?.partnership_details
                          ?.partnership_affiliation === 0
                          ? "Us"
                          : seller?.partnership_details
                              ?.partnership_affiliation === 1
                          ? "Partner"
                          : "N/A"}
                      </TableCell>
                      <TableCell
                      ...
                      >
                        <div className="flex items-center justify-center gap-3">
                          ...
                          <button
                            ...
                            onClick={
                              () => navigate(`/seller-portal/edit/${seller.id}`) 
                            }
                          >
                            <svg
                              width="18"
