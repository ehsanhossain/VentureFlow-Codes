...
const ClosedDeals = ({
  ...
  useEffect(() => {
    const fetchCurrencies = async () => {
      try {
        ...
        const currencies = (Array.isArray(response.data.data) ? response.data.data : response.data) as Currency[];
  ...
  const fetchSellers = useCallback(async (
    ...
    try {
      ...
      const response = await api.get('/api/seller/closed', { params });
  ...
  useEffect(() => {
    api
      .get('/api/countries')
      .then((res) => {
        const formatted = res.data.map((c: ApiCountry) => ({
          id: c.id,
          name: c.name,
                        ...
                        {(() => {
                          try {
                            ...
                            const names = industries
                              .map((item: { name: string }) => truncate(item?.name))
                              .filter(Boolean);
