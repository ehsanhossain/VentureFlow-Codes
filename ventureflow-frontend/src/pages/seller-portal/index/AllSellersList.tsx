...
const AllSellersList = ({
  ...
  useEffect(() => {
    const fetchIndustries = async () => {
      try {
        ...
        const formatted = response.data.map((industry: { id: number; name: string; status: string; sub_industries: Industry[] }) => ({
          id: industry.id,
          name: industry.name,
  ...
  useEffect(() => {
    const delay = setTimeout(() => {
      ...
      fetchSellers(currentPage, queryParams, sortConfig ? `${sortConfig.key}:${sortConfig.direction}` : undefined);

    }, 500);

    return () => clearTimeout(delay);
  }, [currentPage, searchQuery, filters, fetchSellers, sortConfig]);
  ...
  const handleDelete = async (id: number | string) => {
    ...
    try {
      await api.delete(`/api/sellers`, { data: { ids: [id] } });
      fetchSellers();
      showAlert({
        type: 'success',
