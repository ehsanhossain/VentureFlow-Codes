...
const NotInterested = ({
  ...
  interface IndustryWithSub extends Industry {
    status?: string;
    sub_industries?: IndustryWithSub[];
  }

  const [industries, setIndustries] = useState<IndustryWithSub[]>([]);
  ...
  useEffect(() => {
    const fetchIndustries = async () => {
      try {
        ...
        const data = response.data as IndustryWithSub[];
        const formatted: IndustryWithSub[] = data.map((industry) => ({
          id: industry.id,
          name: industry.name,
          status: industry.status,
          sub_industries: (industry.sub_industries || []) as IndustryWithSub[],
        }));
  ...
  useEffect(() => {
    ...
    const formattedSubs = subs.map((sub): Industry & { status: string } => ({
      id: sub.id,
      name: sub.name,
      status: sub.status,
    }));
  ...
  const handleApplySearch = (formValues: FormValues) => {
    ...
    const appliedFilters: Record<string, string | number | string[] | object> = {
  ...
  const [activeFilters, setActiveFilters] = useState<Record<string, string | number | string[] | object>>({});
  ...
  const fetchBuyers = async (page = 1, query = '', filters = {}) => {
    try {
      ...
      const response = await api.get('/api/buyer/unpinned', { params });
  ...
  useEffect(() => {
    api
      ...
      .then((res) => {
        ...
        const formatted: Country[] = res.data.map((c: any) => ({
          id: c.id,
          name: c.name,
  ...
  const handleDeleteData = async (_selectedIdsArray: (number | string)[]) => {
                            ...
                            render={({ field }) => (
                              <div className="w-[300px]">
                                <Dropdown
                                  {...field}
                                  countries={countries}
                                  selected={countries.find((c) => String(c.id) === String(field.value))}
                                  onSelect={(selected) => {
                                    if (!Array.isArray(selected)) {
                                      field.onChange(String(selected.id));
                                    }
                                  }}
                                />
                              </div>
            ...
            <TableBody>
              {loading ? (
                ...
                buyers.map((buyer, index) => {
                  const country = getCountryById(parseInt(String(buyer.company_overview?.hq_country || '0')));
                      ...
                      >
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            ...
                            onClick={() => navigate(`/buyer-portal/edit/${buyer.id}`)}
                          >
                            <svg
                              width="18"
                              height="18"
