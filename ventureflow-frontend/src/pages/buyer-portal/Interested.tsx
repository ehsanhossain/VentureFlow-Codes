...
const Interested = ({
  ...
  useEffect(() => {
    const fetchIndustries = async () => {
      try {
        ...
        const formatted: Industry[] = response.data.map((industry: { id: number; name: string; status: string; sub_industries?: unknown[] }) => ({
          id: industry.id,
          name: industry.name,
  ...
  useEffect(() => {
    ...
    const formattedSubs = subs.map((sub: { id: number; name: string }) => ({
      id: sub.id,
      name: sub.name,
      // status: sub.status, // Removed to match Industry interface
      sub_industries: [],
    }));
  ...
  const fetchBuyers = async (page = 1, query = '', filters = {}) => {
    try {
      ...
      const response = await api.get('/api/buyer/pinned', { params });
  ...
  useEffect(() => {
    const delay = setTimeout(() => {
      fetchBuyers(currentPage, searchQuery);
    }, 500);
  ...
  const handleDelete = async (id: number | string) => {
    ...
    try {
      await api.delete(`/api/buyers`, { data: { ids: [id] } });
      fetchBuyers();
      showAlert({
        type: 'success',
                            ...
                            <div className="flex gap-[13px]">
                              <div className="w-[200px]">
                                <Input
                                  ...
                                  {...register('acquisitionPreference.min')}
                                />
                              </div>
                              ...
                              <div className="w-[200px]">
                                <Input
                                  ...
                                  {...register('acquisitionPreference.max')}
                                />
                              </div>
                            </div>
                          </div>
            ...
            <TableBody>
              {loading ? (
                ...
                buyers.map((buyer, index) => {
                  // Get country by ID before returning JSX
                  const country = getCountryById(parseInt(buyer.company_overview?.hq_country));
