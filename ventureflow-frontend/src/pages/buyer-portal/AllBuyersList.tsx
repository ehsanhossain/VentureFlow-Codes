...
type AllBuyersListProps = {
  onClearFilters: () => void;
};

const AllBuyersList = ({
  onClearFilters,
}: AllBuyersListProps): JSX.Element => {
  ...
  useEffect(() => {
    const fetchIndustries = async () => {
      try {
        ...
        const formatted = response.data.map((industry: { id: number; name: string; status: string; sub_industries?: unknown[] }) => ({
          id: industry.id,
          name: industry.name,
  ...
  useEffect(() => {
    const fetchCurrencies = async () => {
      try {
        ...
        const formatted = currencies.map((currency: { id: number | string; currency_name: string }) => ({
  ...
  const buyerData = useMemo(
    () => [
      {
        buyer_name: 'Kiran Co Ltd.',

        expectedEBITDA: '¥ 3.5B ~ ¥ 5.5B',
        countryFlag: '/group-291565.png',
        hq: 'Italy',
        industry: 'Wholesale',
        investmentRange: '₺110M',
  ...
  const handleSort = (_key: string) => {
    ...
    // Implement sort logic here if needed, or update API params
  };
            ...
            <TableHeader>
              <TableRow className="rounded-lg">
                {tableHeaders.map((header, idx) => {
                  const key = Object.keys(buyerData[0])[idx] as
                    | keyof (typeof buyerData)[0]
                    | undefined;
