...
import { Industry as BaseIndustry, IndustryDropdown } from './components/IndustryDropdown';
...
type ClosedDealsProps = {
  onClearFilters: () => void;
};

const ClosedDeals = ({
  onClearFilters,
}: ClosedDealsProps): JSX.Element => {
  ...
  useEffect(() => {
    const fetchIndustries = async () => {
      try {
        ...
        const formatted = response.data.map((industry: Record<string, unknown>) => ({
          id: industry.id as number,
          name: industry.name as string,
  ...
  useEffect(() => {
    ...
    const formattedSubs = subs.map((sub: Record<string, unknown>) => ({
      id: sub.id as number,
  ...
  useEffect(() => {
    const fetchCurrencies = async () => {
      try {
        ...
        const formatted = currencies.map((currency: Record<string, unknown>) => ({
...
interface Country {
  ...
  const fetchBuyers = async (page = 1, query = '', filters = {}) => {
    try {
      ...
      const response = await api.get('/api/buyer/closed-deals', { params });
  ...
  useEffect(() => {
    api
      .get('/api/countries')
      .then((res) => {
        const formattedCountries = res.data.map((country: Record<string, unknown>) => ({
          ...country,
  ...
  const handleDelete = async (id: number | string) => {
    ...
    try {
      await api.delete(`/api/buyer/${id}`);
      navigate('/buyer-portal');
    } catch {
      showAlert({ type: "error", message: "Failed to delete buyer" });
    }
  };
  ...
  const handleDeleteData = async (selectedIdsArray: (number | string)[]) => {
    ...
    if (window.confirm(confirmMessage)) {
      try {
        ...
        await api.delete(`/api/buyers`, {
          data: { ids: selectedIdsArray as (string | number)[] },
        });
                          ...
                          <Controller
                            ...
                            render={({ field }) => (
                              <div className="w-[300px]">
                                <Dropdown
                                  {...field}
                                  countries={countries as import('./components/Dropdown').Country[]}
                                  selected={field.value as unknown as import('./components/Dropdown').Country | null}
                                  onSelect={(value) => {
                                    if (Array.isArray(value)) {
                                      field.onChange(value[0]?.id?.toString() || '');
                                    } else {
              ...
              <TableRow className="rounded-lg">
                {tableHeaders.map((header, idx) => (
                    <TableHead
                      ...
                      } bg-[#F9F9F9] text-center whitespace-nowrap`}
                    >
            ...
            <TableBody>
              {loading ? (
                ...
                buyers.map((buyer, index) => {
                  const country = getCountryById(parseInt(buyer.company_overview?.hq_country || '0'));
                    ...
                    <TableRow key={index}>
                      ...
                      >
                        <div className="flex items-center gap-2 justify-start">
                          ...
                          <span>{buyer.company_overview?.reg_name ?? '-'}</span>
                        </div>
                      </TableCell>
                        ...
                        {(() => {
                          try {
                            const { min, max } = JSON.parse(
                              buyer.financial_details?.expected_ebitda ?? '{}'
                            );
                            return `${min ?? '-'} - ${max ?? '-'}`;
                          } catch {
                            return '-';
                      ...
                      >
                        {Array.isArray(buyer.company_overview?.main_industry_operations)
                      ...
                      <TableCell
                        className={clsx(
                          'py-[10px] px-6 text-center text-[#064771] text-sm border-t border-b truncate whitespace-nowrap underline',
                          {
                            'border-[#064771] bg-[#F5FBFF]': isSelected,
                            'border-[#E4E4E4] bg-white': !isSelected,
                          }
                      ...
                      >
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            ...
                            onClick={
                              () => navigate(`/buyer-portal/edit/${buyer.id}`) // Assuming you want a similar navigation pattern
                            }
                          >
                            <svg
                              width="18"
