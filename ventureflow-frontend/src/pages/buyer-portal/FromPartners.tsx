...
const FromPartners = ({
  ...
  const fetchBuyers = async (page = 1, query = '', filters = {}) => {
    try {
      ...
      const response = await api.get('/api/buyer/from-partners', { params });
  ...
  const handleDeleteData = (_ids: number[]): boolean => {
  ...
  const handleBulkDeleteClick = async () => {
    ...
    const success = await handleDeleteData(idsToDeleteArray as number[]);
                ...
                buyers.map((buyer, index) => {
                  ...
                  return (
                    <TableRow key={index}>
                      ...
                      >
                        <p>{buyer.partnership_details?.partner.partner_overview.reg_name ?? '-'}</p>
                      </TableCell>
                      ...
                      >
                        <p>
                          {(() => {
                            const budget = buyer.financial_details?.investment_budget;
                            try {
