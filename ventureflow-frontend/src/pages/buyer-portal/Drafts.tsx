...
const Drafts = ({ onClearFilters }: DraftsProps): JSX.Element => {
  ...
  const [subIndustries, setSubIndustries] = useState<IndustryData[]>([]);
  ...
  const fetchBuyers = async (page = 1, query = '', filters = {}) => {
    try {
      ...
      const response = await api.get('/api/buyer/drafts', { params });
  ...
  const handleSort = (_key: keyof (typeof buyerData)[0]) => {
  ...
  const handleDeleteData = (_ids?: any[]) => {
