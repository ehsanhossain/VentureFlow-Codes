...
type FormValues = {
  ...
  partnerSideContactPersonName: string;
...
const CompanyOverview: React.FC = () => {
  ...
  const { fields: shareholderFields, append: appendShareholder } = useFieldArray({
    control,
    name: 'shareholders',
  });
...
`);
  ...
  useEffect(() => {
    const fetchIndustries = async () => {
      try {
        ...
        const formatted = response.data.map((industry: Industry) => ({
          id: industry.id,
          name: industry.name,
  ...
  interface PartnerData {
    ...
    our_contact_person: number | string;
    partner_id: string;
  }

  const [partnerData, setPartnerData] = useState<PartnerData | null>(null);
  const { id: paramId } = useParams();
  const storedPartnerId = React.useMemo(() => localStorage.getItem('partner_id'), []);
  const id = paramId ?? storedPartnerId ?? null;
  ...
  useEffect(() => {
    ...
    const fetchPartnerOverview = async () => {
      try {
        const response = await api.get(`/api/partner-overviews/${id}`);
        setPartnerData(response.data.data);
      } catch {
        showAlert({ type: "error", message: "Failed to fetch partner data" });
      }
    };
  ...
  useEffect(() => {
    ...
    const generateDealroomId = async () => {
      ...
      try {
        const response = await api.get(`/api/partner/get-last-sequence?country=${countryAlpha}`);
        const lastSequence = response.data.lastSequence;

      

        if (typeof lastSequence !== 'number') {
          showAlert({ type: "error", message: "Invalid data received for last sequence" });
          throw new Error('Invalid data received for last sequence.');
        }

        const nextSequence = lastSequence + 1;
        const formattedSequence = String(nextSequence).padStart(2, '0');
        const newDealroomId = `${countryAlpha}-P-${formattedSequence}`;

        const buyerId = partnerData?.partner_id;
        const prefix = buyerId?.split('-')[0];

        if (id && prefix == countryAlpha) {
          setValue('dealroomId', partnerData?.partner_id || '');
        } else {
          setValue('dealroomId', newDealroomId);
        }
      } catch {
        showAlert({ type: "error", message: "Error generating dealroom ID" });
      }
    };

    generateDealroomId();
  }, [originCountry, partnerData, setValue, id]);
  ...
  useEffect(() => {
    ...
    setValue('companyName', partnerData.reg_name || '');
    setValue('companyType', partnerData.company_type || '');

    const yearFoundedValue = partnerData.year_founded;
    const yearAsNumber = Number(yearFoundedValue);
    setValue(
      'yearFounded',

      yearFoundedValue && !isNaN(yearAsNumber) ? new Date(yearAsNumber, 0, 1) : null
    );

    if (partnerData.niche_industry !== null && partnerData.niche_industry !== undefined) {
      setValue('priorityIndustries', partnerData.niche_industry);
    }

    setValue('fullTimeEmployeeCounts', partnerData.current_employee_count);
    setValue('potentialSynergries', partnerData.details || '');
    setValue('companyPhoneNumber', partnerData.company_phone || '');
    setValue('companyEmail', partnerData.company_email || '');

    setValue(
      'shareholders',
      (partnerData.shareholder_name || []).map((name: string) => ({ name }))
    );
    setValue(
      'hqAddresses',
      (partnerData.hq_address || []).map((address: string) => ({ address }))
    );
    setValue(
      'contactPersons',
      (partnerData.contact_person_phone || []).map((phoneNumber: string) => ({
        phoneNumber,
      }))
    );

    setValue('partnerSideContactPersonName', partnerData.contact_person_name || '');
    setValue('designationAndPosition', partnerData.contact_person_position || '');
    setValue('emailAddress', partnerData.contact_person_email || '');
    setValue('phoneNumber', partnerData.contact_person_phone?.[0] || '');

    setValue('websiteLink', partnerData.website || '');
    setValue('linkedinLink', partnerData.linkedin || '');
    setValue('twitterLink', partnerData.twitter || '');
    setValue('facebookLink', partnerData.facebook || '');
    setValue('instagramLink', partnerData.instagram || '');
    setValue('youtubeLink', partnerData.youtube || '');

    setValue('noPICNeeded', partnerData?.no_pic_needed || false);

    const origin = countries.find((c) => c.id === partnerData.hq_country);
    setValue('originCountry', origin || null);

    const main = (partnerData.main_countries || [])
      .map((id: number) => countries.find((c) => c.id === id))
      .filter((c): c is Country => !!c);
    setValue('mainCountries', main);
    setValue('details', partnerData.details || '');
    setValue('profilePicture', null);

    const emp = employees.find((e) => String(e.id) === String(partnerData.our_contact_person));

    setValue('pic', emp ? emp.id.toString() : null);

    if (id) {
      localStorage.setItem('partner_id', id);
    }
  }, [partnerData, countries, id, setValue, employees]);
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    formData.append('reg_name', data.companyName || '');
    formData.append('hq_country', data.originCountry?.id?.toString() || '');
    formData.append('company_type', data.companyType || '');
    formData.append('year_founded', data.yearFounded ? data.yearFounded.getFullYear().toString() : '');

    formData.append('no_pic_needed', data.noPICNeeded ? '1' : '0');

    formData.append('main_countries', JSON.stringify((data.mainCountries ?? []).map((c) => c.id)));
    formData.append(
      'niche_industry',
      JSON.stringify(
        Array.isArray(data.priorityIndustries) ? data.priorityIndustries : [data.priorityIndustries]
      )
    );
    formData.append('current_employee_count', data.fullTimeEmployeeCounts || '');
    formData.append('our_contact_person', data.pic || '');

    formData.append('company_email', data.companyEmail || '');
    formData.append('company_phone', data.companyPhoneNumber || '');
    formData.append(
      'hq_address',
      JSON.stringify((data.hqAddresses ?? []).map((item) => item.address))
    );
    formData.append(
      'shareholder_name',
      JSON.stringify((data.shareholders ?? []).map((item) => item.name))
    );

    formData.append('contact_person_name', data.partnerSideContactPersonName || '');
    formData.append('contact_person_position', data.designationAndPosition || '');
    formData.append('contact_person_email', data.emailAddress || '');
    formData.append(
      'contact_person_phone',
      JSON.stringify((data.contactPersons ?? []).map((item) => item.phoneNumber))
    );

    formData.append('website', data.websiteLink || '');
    formData.append('linkedin', data.linkedinLink || '');
    ...
    formData.append('youtube', data.youtubeLink || '');

    formData.append('details', data.details || '');
    formData.append('partner_id', data.dealroomId || '');
    if (data.profilePicture?.[0]) {
      formData.append('partner_image', data.profilePicture[0]);
    }

    try {
      const response = await api.post('/api/partner-overviews', formData, {
        headers: {
      ...
      setActiveTab('partnership-structure');
    } catch {
      showAlert({ type: 'error', message: 'Error submitting form' });
    }
  };
  ...
  const onDraftCancel = async (data: FormValues) => {
    ...
    try {
      ...
      showAlert({ type: 'error', message: 'Error saving draft' });
    }
  };
      ...
      <div className="pl-[25px] font-poppins">
        <div className="flex gap-[70px] ">
          <div className="flex flex-col items-start gap-4 p-0 pt-12 pl-7 rounded-none">
            ...
            />
            <div className="flex items-center gap-2">
              <Label text="Main Countries/Customer Base" required />
            ...
            <div className="flex items-center gap-3 p-[10px_20px_10px_20px]rounded-md w-full sm:w-80 md:w-96 lg:w-[500px]">
              <div className="w-full bg-transparent text-gray-600 text-sm font-medium outline-none placeholder-gray-400 max-h-10 pr-2">
                <Input
                  ...
                  placeholder="Copy & Paste the Partner’s X (Twitter) Link here"
                  leftIcon={
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="20"
          ...
          <div className="flex flex-col items-start gap-4 p-0 pt-12 pl-7 rounded-none ">
            ...
            <Label text=" Partner's Contact Person Name" />

            <div className="flex items-center gap-3 p-[10px_20px_10px_20px]rounded-md w-full sm:w-80 md:w-96 lg:w-[500px]">
              <div className="w-full bg-transparent text-gray-600 text-sm font-medium outline-none placeholder-gray-400 max-h-10 pr-2">
                <Input
                  {...register('partnerSideContactPersonName', {})}
          ...
          <div className="absolute left-[1300px] top-[245px] w-[1px] bg-gray-300 h-[1500px] mt-[100px]" />
          <div className=" mt-[80px] ml-[30px]">
            <DealroomID
              ...
              label="Partner ID"
              description="The Partner ID (e.g., TH-P-1) signifies the location (TH = Thailand), the entity type (P = Partner), and a unique sequential number (1 = first partner from Thailand)."
              onSave={() => {
                showAlert({
                  type: 'success',
                  message: 'Partner ID saved successfully!',
                });
              }}
              iconSrc={
                <svg
                ...
                <div className="w-[190px] justify-center gap-[15px] left-[20px] flex flex-col items-center relative">
                  <input
                    ...
                    ref={(e) => {
                      ...
                      if (fileInputRef) {
                         // @ts-expect-error - fileInputRef.current is readonly but we need to set it
                ...
                <div className="text-[#30313D] text-sm">
                  <ul className="pl-5">
                    <li>
                      ...
                      successfully create a new partner profile.
                    </li>
                    <br />
                    <li>
                    ...
                    <li>
                      ...
                      identification in the partner portal (max file size: 1MB).
                    </li>
                  </ul>
                </div>
              </div>
          ...
          <div className="flex justify-between items-center flex-row gap-[916px] mt-5">
            ...
            <div className="flex justify-start items-center flex-row gap-4 ml-[-120px]">
              ...
              <div className="flex justify-start items-end flex-row gap-[10.06532096862793px] h-[34px]">
                <button
                  type="submit"
                  className="min-w-[120px] flex justify-center items-center flex-row gap-1.5 py-[5px] px-3 bg-[#064771] rounded-full h-[34px] disabled:opacity-50"
                  disabled={isSubmitting}
                >
                  {isSubmitting ? (
                    ...
                    <>
                      <span className="text-white whitespace-nowrap">Save and Next</span>
                      <svg
                        width="8"
                        height="14"
                        viewBox="0 0 8 14"
