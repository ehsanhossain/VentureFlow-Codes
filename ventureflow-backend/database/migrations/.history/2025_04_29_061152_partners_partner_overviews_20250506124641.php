...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create('partners_partner_overviews', function (Blueprint $table) {
            ...
            // Basic Details
            $table->string('reg_name', 255);                           // Company Registered Name
            $table->foreignId('hq_country')->constrained('countries');                   // HQ/Origin Country
            $table->string('company_type', 100)->nullable();                  // Company Type
            $table->string('year_founded', 100)->nullable();                  // Year Founded
            $table->json('main_countries');

            $table->json('niche_industry');                   // Niche / Priority Industry
            $table->string('current_employee_count', 100);
            $table->foreignId('our_contact_person')->constrained('employees');              // Employee Incharge
